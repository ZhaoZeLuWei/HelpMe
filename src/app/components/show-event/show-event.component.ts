...
function _tryAMapPos(): Promise<{ lng: number; lat: number } | null> {
  return new Promise((resolve) => {
    ...
    AMap.plugin(['AMap.Geolocation'], () => {
      const g = new AMap.Geolocation({ enableHighAccuracy: false, timeout: 10000, maximumAge: 60000 });
      g.getCurrentPosition((status: string, r: any) => {
        if (status === 'complete') resolve({ lng: r.position.getLng(), lat: r.position.getLat() });
        else resolve(null);
      });
    });
  });
...
function _initPos(): Promise<{ lng: number; lat: number } | null> {
  ...
  _posPromise = (async () => {
    ...
    if (p) { _cachedPos = { ...p, timestamp: Date.now() }; return p; }
    if (!navigator.geolocation) return null;
    return new Promise((r) => {
      navigator.geolocation.getCurrentPosition(
        (pos) => { const res = { lng: pos.coords.longitude, lat: pos.coords.latitude }; _cachedPos = { ...res, timestamp: Date.now() }; r(res); },
        () => r(null),
...
/** 获取用户位置 */
export async function getUserPosition(): Promise<{ lng: number; lat: number } | null> {
  if (_cachedPos && Date.now() - _cachedPos.timestamp < 30 * 60 * 1000) return { lng: _cachedPos.lng, lat: _cachedPos.lat };
  if (_posPromise) return _posPromise;
  return _initPos();
}
...
export function isOnlineService(address: string): boolean {
  return address?.includes('线上服务') || address?.includes('在线服务') || address?.includes('线上');
}
...
async function _ensureGeo() {
  ...
  _geoPromise = new Promise((r) => {
    ...
    AMap.plugin(['AMap.Geocoder'], () => { _geocoder = new AMap.Geocoder({ city: '全国', radius: 1000 }); r(); });
  });
  return _geoPromise;
}

/** 地址转坐标 */
export async function resolveAddress(addr: string): Promise<{ lng: number; lat: number } | null> {
  if (!addr || isOnlineService(addr)) return null;
  const c = _addrCache.get(addr);
  if (c) return c;
  const m = addr.match(/^([^\s·]+)[·\s]/);
  if (m && CITY[m[1] as keyof typeof CITY]) { _addrCache.set(addr, CITY[m[1] as keyof typeof CITY]); return CITY[m[1] as keyof typeof CITY]; }
  await _ensureGeo();
  ...
  return new Promise((r) => {
    _geocoder.getLocation(addr, (s: string, res: any) => {
      if (s === 'complete' && res.geocodes?.length) {
        ...
        const coords = { lng: loc.location.getLng(), lat: loc.location.getLat() };
        _addrCache.set(addr, coords); r(coords);
      } else r(null);
    });
  });
}
